...
// same floor — a flash that drops out of legibility is a gauge that keeps
// disappearing. It brightens rather than dims for exactly that reason, which
// the first check below is what proves.
for (const phase of [0, 0.25, 0.5, 0.75]) {
  check(`red gauge mid-flash (phase ${phase})`, contrast(gaugeColor(100, phase), track), 3.0);
}
// And it has to be a flash: the same colour below the threshold, a swing wide
...
// from across a room, which is why the floor here is 40 instead: two fades
// that only survived a colour-picker went unnoticed on the actual deck.
if (gaugeColor(CONTEXT_CRITICAL - 1, 0.5) !== usageColor(CONTEXT_CRITICAL - 1)) {
  console.error("FAILED: an amber gauge must not pulse");
  failed = true;
}
check("the red gauge's flash is wide enough to see", deltaE(gaugeColor(100, 0), gaugeColor(100, 0.5)), 40);
