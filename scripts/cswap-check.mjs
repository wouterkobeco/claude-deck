...
const sequence = { activeAccountNumber: 2, accounts: { 3: { email: "b@x.com" }, 2: { email: "a@x.com" } } };
const usage = {
  accounts: {
...
assert.deepEqual(parsed.map((a) => a.slot), ["2", "3"], "slot order, not object order");
assert.deepEqual(parsed[0], {
  slot: "2", email: "a@x.com", name: "a", active: true,
  session: 0, week: 29, sessionResetsAt: null, weekResetsAt: "2026-08-26T06:00:00+00:00",
});
assert.deepEqual([parsed[1].active, parsed[1].session, parsed[1].week], [false, null, null], "no usage yet is unknown, not zero");
