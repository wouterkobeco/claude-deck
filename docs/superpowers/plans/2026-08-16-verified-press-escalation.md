...
**Type consistency:** `readWindowStates(dir?)` returns `{pid, folders, focused, activeSessionId}` in T1 and is consumed under those exact names in T2's `win()` helper and in `isRepeatPress`. The file shape written in T3.3 matches what T1.4 parses field for field. `isAlive` is exported in T1.3 and imported in T1.4. `isRepeatPress(previous, press, windows = [])` is defined in T2.3 and called with three arguments in T2.5.
